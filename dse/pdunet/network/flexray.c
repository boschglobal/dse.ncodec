...
void pdunet_flexray_parse_pdu(PduNetwork* net, PduItem* pdu, void* n)
{
    ...
    if (md) {
        pdunet_flexray_parse_pdu_metadata(net, pdu, md);
    }
}
...
void pdunet_flexray_config(PduNetwork* net)
{
    ...
    log_notice(net->log, "PDU Net: FlexRay Network Config: %s", net->name);
...
void pdunet_flexray_lpdu_rx(PduNetwork* net)
{
    while (1) {
        ...
        case NCodecPduFlexrayMetadataTypeStatus:
            ...
            log_debug(net->log,
                "  FlexRay: State channel[0]: poc_state=%s, "
            ...
            if (net->network.vtable.flexray.cycle != s.cycle) {
                ...
                if (previous_offset != net->schedule.epoch_offset) {
                    log_info(net->log,
                        "Epoch shift: %f -> %f (cycle=%u, macrotick=%u)",
...
void pdunet_flexray_parse_network_metadata(PduNetwork* net, YamlNode* md)
{
    ...
    pdunet_schema_load_object(net, md, net->network.metadata.config, md_fr_spec,
        ARRAY_SIZE(md_fr_spec));
}
    ...
    PduNetwork* net, PduItem* pdu, YamlNode* md)
{
    ...
    static const PdunetSchemaFieldMapSpec mode_map[] = {
    ...
    pdunet_schema_load_object(
        net, md, pdu->metadata.config, md_fr_spec, ARRAY_SIZE(md_fr_spec));
}
