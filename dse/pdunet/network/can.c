...
void pdunet_can_parse_pdu(PduNetwork* net, PduItem* pdu, void* n)
{
    ...
    if (md) {
        pdunet_can_parse_pdu_metadata(net, pdu, md);
    }
}
...
void pdunet_can_config(PduNetwork* net)
{
    ...
    log_notice(net->log, "PDU Net: CAN Network Config: %s", net->name);
    ...
    for (size_t i = 0; i < vector_len(&net->pdus); i++) {
        ...
        log_notice(net->log,
            "PDU Net:   CAN: PDU[%u] id=0x%03X, len=%u, dir=%s, "
            "name=%s%s",
...
void pdunet_can_parse_pdu_metadata(PduNetwork* net, PduItem* pdu, YamlNode* md)
{
    ...
    static const PdunetSchemaFieldMapSpec ff_map[] = {
    ...
    static const PdunetSchemaFieldMapSpec ft_map[] = {
    ...
    static const PdunetSchemaFieldSpec md_can_spec[] = {
        // clang-format off
    ...
    pdunet_schema_load_object(
        net, md, pdu->metadata.config, md_can_spec, ARRAY_SIZE(md_can_spec));
}
