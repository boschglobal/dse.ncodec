...
{
    PdunetMock* mock = __create_mock("resources/model/pdunet_lua.yaml");
    *state = mock;
    return 0;
}


static int test_setup_container(void** state)
{
    PdunetMock* mock = __create_mock("resources/model/pdunet_container.yaml");
    *state = mock;
    return 0;
}
...
{
    PdunetMock* mock = __create_mock("resources/model/pdunet_secured.yaml");
    *state = mock;
    return 0;
}
...
void test_pdu_flexray_network_parse(void** state)
{
    ...
    assert_non_null(net);
    assert_non_null(net->ncodec);
    assert_non_null(net->doc);
    assert_non_null(net->lua.lua_state);
    assert_ptr_equal(net->ncodec, mock->ncodec);
    assert_ptr_equal(net->doc, mock->doc);
    assert_ptr_equal(net->lua.lua_state, mock->L);
...
void test_pdunet_transform(void** state)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(pdu_checks); i++) {
        ...
        log_info(&dlog, "PDU Check[%u] idx=%u", i, c.idx);
    }
    ...
    for (size_t i = 0; i < ARRAY_SIZE(checks); i++) {
        ...
        log_info(&dlog,
            "Check[%u] name=%s, pdu_idx=%u, signal_idx=%u\n"
            "         factor=%f, offset=%f, min=%f, max=%f\n"
            "         encode_ref=%u, decode_ref=%u, start_bit=%u, "
            "length_bits=%u",
            i, c.name, c.pdu_idx, c.signal_idx, c.factor, c.offset, c.min,
            c.max, c.encode_ref, c.decode_ref, c.start_bit, c.length_bits);
...
//         assert_int_equal(*(int*)vector_at(&net->matrix.signal.decode, c.idx, NULL), c.decode_ref); // NOLINT
    ...
    for (size_t i = 0; i < ARRAY_SIZE(range_checks); i++) {
        ...
        log_info(&dlog, "Range Check[%u] dir=%u, offset=%u, length=%u", i,
            c.dir, c.offset, c.length);
...
void test_pdunet_ncodec_pdu(void** state)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(checks); i++) {
        ...
        log_info(&dlog, "PDU Check[%u] idx=%u, name=%s, id=%u, length=%u", i,
            c.idx, c.name, c.pdu_id, c.payload_length);
...
void test_pdunet_linear_part_tx(void** state)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(checks); i++) {
        matrix_check c = checks[i];
        log_info(&dlog,
            "Check TX[%u] idx=%u, name=%s, phys=%f, raw=%u (expect)", i, c.idx,
            c.name, c.phys, c.raw);
...
void test_pdunet_linear_part_rx(void** state)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(checks); i++) {
        matrix_check c = checks[i];
        log_info(&dlog,
            "Check RX[%u] idx=%u, name=%s, phys=%f, raw=%u (expect)", i, c.idx,
            c.name, c.phys, c.raw);
...
void test_pdunet_lua_tx(void** state)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(signal_checks); i++) {
        ...
        log_info(&dlog, "Check[%u] name=%s, idx=%u, pdu_idx=%u, signal_idx=%u",
            i, c.name, c.idx, c.pdu_idx, c.signal_idx);
        // Check the name is as expected.
...
void test_pdunet_container_tx(void** state)
{
    // dlog.level = LOG_DEBUG;
    PdunetMock* mock = *state;
    ...
    mock->net = net;  // Teardown will destroy.
    assert_non_null(net);
    assert_int_equal(vector_len(&net->pdus), 8);
    assert_int_equal(vector_len(&net->matrix.pdu), 8);

    PduItem pdu;
    vector_at(&net->pdus, 0, &pdu);
    assert_int_equal(pdu.id, 400);
    assert_int_equal(pdu.length, 24);
    assert_int_equal(pdu.dir, 2);
    assert_non_null(pdu.name);
    assert_string_equal(pdu.name, "LPDU-TX");
    assert_int_equal(pdu.container.header, 2);
    assert_int_equal(pdu.container.id, 0);

    vector_at(&net->pdus, 1, &pdu);
    assert_int_equal(pdu.id, 401);
    assert_int_equal(pdu.length, 8);
    assert_int_equal(pdu.dir, 2);
    assert_non_null(pdu.name);
    assert_string_equal(pdu.name, "IPDU-TX-1");
    assert_int_equal(pdu.container.header, 0);
    assert_int_equal(pdu.container.id, 400);
    assert_int_equal(pdu.container.priority, 50);

    // Set some signal values and then Pack.
    matrix_check values[] = {
        { .idx = 3, .name = "SIG_1", .phys = 1, .raw = 1 },
        { .idx = 4, .name = "SIG_2", .phys = 2, .raw = 2 },
        { .idx = 5, .name = "SIG_3", .phys = 3, .raw = 3 },
    };
    for (size_t i = 0; i < ARRAY_SIZE(values); i++) {
        matrix_check c = values[i];
        log_info(&dlog,
    ...
    // Set checksum=0 so that following detects change (set in pdunet_create).
    pdunet_visit(net, NULL, pdunet_visit_clear_checksum, NULL);
    ...
    uint8_t payload[24] = {
        ...
        [13] = 0x01,
        [14] = 0x93,
        [15] = 0x08,  // header - 8bit : 8
        [16] = 0x03,  // SIG_3" : 3
    };
    assert_memory_equal(o->ncodec.pdu.payload, payload, 24);
    int count = 0;
...
void test_pdunet_container_rx(void** state)
{
    // dlog.level = LOG_DEBUG;
    PdunetMock* mock = *state;
    ...
    PduItem pdu;
    vector_at(&net->pdus, 4, &pdu);
    assert_int_equal(pdu.id, 400);
    assert_int_equal(pdu.length, 24);
    assert_int_equal(pdu.dir, 1);
    assert_non_null(pdu.name);
    assert_string_equal(pdu.name, "LPDU-RX");
    assert_int_equal(pdu.container.header, 2);
    assert_int_equal(pdu.container.id, 0);

    vector_at(&net->pdus, 5, &pdu);
    assert_int_equal(pdu.id, 401);
    assert_int_equal(pdu.length, 8);
    assert_int_equal(pdu.dir, 1);
    assert_non_null(pdu.name);
    assert_string_equal(pdu.name, "IPDU-RX-1");
    assert_int_equal(pdu.container.header, 0);
    assert_int_equal(pdu.container.id, 400);
    assert_int_equal(pdu.container.priority, 0);

    // Pack the raw values and check the payload.
    uint8_t** _ = vector_at(&net->matrix.payload, 0, NULL);
    uint8_t*  payload = *_;
    payload[0] = 0x00;  // header - 24bit : 402
    payload[1] = 0x01;
    payload[2] = 0x92;
    payload[3] = 0x08;   // header - 8bit : 8
    payload[4] = 0x02;   // SIG_2" : 2
    payload[12] = 0x00;  // header - 24bit : 403
    payload[13] = 0x01;
    payload[14] = 0x93;
    ...
    // Check some signal values.
    matrix_check values[] = {
        { .idx = 0, .name = "SIG_4", .phys = 0, .raw = 0 },
        { .idx = 1, .name = "SIG_5", .phys = 2, .raw = 2 },
        { .idx = 2, .name = "SIG_6", .phys = 3, .raw = 3 },
    };
    for (size_t i = 0; i < ARRAY_SIZE(values); i++) {
        matrix_check c = values[i];
        log_info(&dlog,
...
void test_pdunet_secured_pdu_tx(void** state)
{
    ...
    for (size_t i = 0; i < ARRAY_SIZE(checks); i++) {
        ...
        log_info(&dlog,
            "Check TX[%u] idx=%u, name=%s, needs_tx=%d, "
...
int run_pdunet_network_tests(void)
{
    void* s = test_setup;
    ...
    return cmocka_run_group_tests_name("PDU Net::Network", tests, NULL, NULL);
}
